...
namespace VRStandardAssets.Utils
{
    ...
    public class VREyeRaycaster : MonoBehaviour
    {
		...
		[SerializeField] private MenuRay m_end;
        ...
        private void EyeRaycast()
        {
			...
			RaycastHit hitInfo;
			Ray ray=	m_end.GetRay();
            RaycastHit hit;
			if(Physics.Raycast(ray,out hitInfo)){
				if(hitInfo.collider.tag=="color"){
					if(canChange){
						canChange = false;
						if(colorNumber==1){
							hitInfo.transform.GetChild (0).gameObject.GetComponent<Image> ().color = red;
						}else if(colorNumber==2){
							hitInfo.transform.GetChild (0).gameObject.GetComponent<Image> ().color = blue;
						}else if(colorNumber==3){
							hitInfo.transform.GetChild (0).gameObject.GetComponent<Image> ().color = yelloe;
						}
						colorNumber++;
						if(colorNumber>=4){
							colorNumber = 1;
